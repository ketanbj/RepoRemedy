...
2. Choose proposal IDs. Run `shadowRepoRemedy publish runs/first --select ID1,ID2`.
...
4. Run `shadowRepoRemedy feedback runs/first` to collect decisions, reviews and comments.
...
When the account cannot push to a target, shadowRepoRemedy creates/uses its fork and
...
`shadowRepoRemedy: accepted`, `shadowRepoRemedy: declined`, `shadowRepoRemedy: needs-adjustment`, or
`shadowRepoRemedy: too-difficult`. Only OWNER/MEMBER/COLLABORATOR decisions are classified;
...
Legacy `reporemedy:` feedback comments remain recognized. Persisted publication
markers and remedy branch prefixes retain their original `reporemedy` spelling
so duplicate detection and interrupted-publication recovery keep working.
